...
int knapsack(vector<int>&value,vector<int>&weights,int n,int capacity){
     ...
     else if(arr[n][capacity] != -1){
         return arr[n][capacity];
     }
     else if(weights[n-1]<=capacity){
          arr[n][capacity] = max(value[n-1]+knapsack(value,weights,n-1,capacity-weights[n-1]),knapsack(value,weights,n-1,capacity));
          return arr[n][capacity];
     }
     else{
          arr[n][capacity] = knapsack(value,weights,n-1,capacity);
          return arr[n][capacity];
...
int main(){
    ...
    fill(&arr[0][0],&arr[0][0]+(1000)*(1000),-1);
    int capacity = 50;
